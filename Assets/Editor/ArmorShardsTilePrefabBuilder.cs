...
// Строит Resources/UI/ArmorShardsTile.prefab — отдельная от CurrencyBalanceTile карточка под
// CurrencyType.ArmorShards (кликабельная, см. ArmorShardsTileUI). Пропорции те же, что у
// CurrencyBalanceTile — подогнаны под cellSize {149,200} общей сетки ItemCollectionUI.
public static class ArmorShardsTilePrefabBuilder
{
    private const string OutputPath = "Assets/Resources/UI/ArmorShardsTile.prefab";

    [MenuItem("Tools/Build ArmorShardsTile Prefab")]
    public static void BuildPrefab()
    {
        var root = new GameObject("ArmorShardsTile", typeof(RectTransform));
        ...
        var tileUI = root.AddComponent<ArmorShardsTileUI>();
        tileUI.icon = icon;
        tileUI.label = label;
        ...
        Debug.Log($"ArmorShardsTile prefab saved to {OutputPath}");
    }
}
#endif
